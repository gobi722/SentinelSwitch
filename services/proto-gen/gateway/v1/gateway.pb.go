...
//
// Authorization:
//   Every call must carry a valid API key in the gRPC metadata key
//   "x-api-key". The API Gateway validates it against the api_clients
//   registry (Postgres, Redis-cached) before the handler runs, and derives
//   a verified client_id (the caller/integrator identity) from the key —
//   never from the request body. This is distinct from merchant_id, which
//   remains client-supplied business data identifying which merchant the
//   transaction is FOR, not who is calling the API.
// =============================================================================
...
type TransactionRequest struct {
	...
	// ------------------------------------------------------------------
	// Merchant context
	// merchant_id identifies which merchant the transaction is FOR.
	// Client-supplied. Distinct from the caller's own identity (client_id),
	// which is derived server-side from the authenticated x-api-key and is
	// never present on this request.
	// ------------------------------------------------------------------
